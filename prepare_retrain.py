...
def col_fn(batchdata):
    ...
    a = torch.from_numpy(np.stack(padded_td))
    b = torch.from_numpy(np.stack(static))
    c = torch.from_numpy(np.stack(padded_label))
    d = torch.from_numpy(np.stack(stayids))
    e = torch.from_numpy(np.stack(mask))
